...
class main:
	...
	def combat(self, al, dl, aw, dw, ae, de):
		...
		print ("%s %i vs %s %i Result: %i" 
			% (self.nameatk, al, self.namedef, dl, combatresult))

		if combatresult>0:
			self.combat_out.append("\n%s[%i] %s %s[%i]" 
				% (self.nameatk, atkres, random.choice(db.winp), self.namedef, defres))
			self.combat_exp_gain(self.nameatk, self.namedef)	
			self.level_check(self.jidatk, self.nameatk)
		elif combatresult<=0:
			self.combat_out.append("\n%s[%i] %s %s[%i]" 
				% (self.nameatk, atkres, random.choice(db.losep), self.namedef, defres))
	...
	def timed_weapon_gain(self):
		...
		if weapon_roll > self.pdata["WEAPON"]:
			...
			self.parent.channel_message("\n%s has gained a %s!" 
				% (self.name, db.wlist[weapon_roll]["name"]))
	...
	def weapon_decay(self, jid, name):
		...
		if self.pdata["WEAPON"] != 0:
			if self.pdata["WEAPON_DUR"] <= 1:
				self.combat_out.append("\n%s's %s has broken!" 
					% (name, db.wlist[self.pdata["WEAPON"]]['name']))
				self.pdata["WEAPON"] = 0
	...
	def SKILL_caustic_armor(self, jid1, name1, jid2, name2):
		...
		if self.pdata["WEAPON"] != 0:
			if self.pdata["WEAPON_DUR"] <= 3:
				self.combat_out.append("\n%s's caustic armor destroys %s's weapon!" 
					% (name2, name1))
				self.pdata["WEAPON"] = 0
				self.pdata["WEAPON_DUR"] = 0
			else:
				self.pdata["WEAPON_DUR"] -= 3
				self.combat_out.append("\n%s' caustic armor corrodes %s's weapon!" 
					% (name2, name1))
			...
			if self.check(jid):
				...
				if self.pdata["EFFECT"] != 0:
					if self.pdata["EFFECT_DUR"] == 1:
						self.parent.channel_message("\n%s's %s has worn off!" 
							% (name, db.elist[self.pdata["EFFECT"]]['name']))
						self.pdata["EFFECT"] = 0
	...
	def timed_effects_gain(self):
		...
		if self.pdata["EFFECT"] == 0:
			...
			self.write_data(self.jid)
			self.parent.channel_message("\n%s is empowered with %s!" 
				% (self.name, db.elist[1]['name']))
	...
	def level_check(self, jid, name):
		...
		if int(self.pdata["EXP"]) > int(self.pdata["EXP2L"]):
			...
			self.parent.channel_message("\n%s has reached level %i!" %
				(name, self.pdata["LEVEL"]))
		self.write_data(jid)
	...
	def check(self, jid):
		try:
			f=open("/home/pi/koahiRPG/pfiles/"+jid+".txt" ,"r")			
			f.close()
			return 1		
		except:
			return 0
	...
	def create(self, jid, msg):
		...
		if self.check(jid):
			...
			f=open("/home/pi/koahiRPG/pfiles/"+jid+".txt" ,"w+")
			f.write("LEVEL:1\n")
			f.write("EXP:0\n")
			f.write("EXP2L:200\n")
			f.write("WEAPON:0\n")		
